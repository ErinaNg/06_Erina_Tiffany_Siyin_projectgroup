...
public class Eri_malechara : MonoBehaviour
{
    ...
    private void Move()
    {
        isGrounded = Physics.CheckSphere(transform.position, groundCheckDistance, groundMask);  //return true if grounded
        if(isGrounded && velocity.y < 0)  //stop applying gravity when grounded
        {
            velocity.y = -2f;
        }
        ...
        if(isGrounded)
        {
            if (moveDirection != Vector3.zero && !Input.GetKey(KeyCode.LeftShift))  //Walk //not pressing left shift
            {
                Walk();
            }
